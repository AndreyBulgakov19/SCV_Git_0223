...
## 4 Работа с чужим репозиторием на локальном компьютере с помощью `Pull request`
## 4.1 Связывание удалённого и локального репозиториев
...
git push -u origin main
```
## 4.2 Клонирование копии чужого репозитория на локальный компьютер
...
```
## 4.3 Создание новой ветки на локальном компьютере
...
```
## 4.7 На сайте GitHub выполнить `Pull request`
Переходим на сайт GitHub и в своём аккаунте находим копию чужого репозитория. Убеждаемся, что все изменения с локального репозитория были получены, то кликаем по кнопке `Pull request`. Переходим в чужой аккаунт и смотрим предлагает ли Git сохранить полученные изменения с чужого аккаунта.
