...
* После всех изменений с помощью команды

    ```
    git push -u origin <новая ветка>
    ```
    создаём новую ветку в удалённом репозитории

После добавления <новой ветки> в удаленном репозитории для обновления информации достаточно будет команды 
...
Команды :
```
git remote - показывает список текущих on-line репозиториев
git remote -v - показывает привязку удалённых репозиториев
```
После того, как скопировали новую ветку в свой  удалённый репозиторий можно произвести `pull request`

```
pull request - предложение изменения кода в чужом репозитории
```
Для того чтобы сделать `pull request` после добавленных изменений в удаленном репозитории нужно:
* в своём репозитории зайти во вкладку `Pull request`
* выбрать  `new pull request`
* в вкладке своего репозиторий `compare` выбрать <новую ветку> с предлагаемыми изменениями
* появится информация о том, что ветка может быть слита `Able to merge. These branches can be automatically merged.`

И предложена возможность сделать изменения

 `Create pull request`

 После чего будут отправлены изменения автору.
